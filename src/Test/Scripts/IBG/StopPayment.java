...
public class StopPayment extends BaseClass {
    ...
    public void stopPaymentCheque(Map<String, String> testData) throws IOException {
        ...
        acpOverride.click();
        PageObject.txnValidate("IBG_stopPaymentCheque");
        String txn = PageObject.getTxn();
        System.out.println("Txn is:  "+txn);

    }

    @Test(groups = {"IBGInputter"}, priority = 2,dataProvider = "excelDataStopPaymentChequeSeries")
    public void stopPaymentChequeSeries(Map<String, String> testData) throws IOException {
        ...
        acpOverride = driver.findElement(By.xpath("//tr/td/a[text()='Accept Overrides']"));
        acpOverride.click();
        PageObject.txnValidate("IBG_stopPaymentChequeSeries");
        String txn = PageObject.getTxn();
        System.out.println("Txn is:  "+txn);
    }
    ...
    public Object[][] readExcelData2() throws IOException {
        FileInputStream fis = new FileInputStream(FILE_PATH);
        Workbook workbook = new XSSFWorkbook(fis);
        Sheet sheet = workbook.getSheet("stopPaymentChequeSeries"); // Assuming data is in the first sheet
        int rowCount = sheet.getPhysicalNumberOfRows();
        int colCount = sheet.getRow(0).getPhysicalNumberOfCells();
        Object[][] data = new Object[rowCount - 1][1]; // One column to store the HashMap
